...
const API_URL = 'https://finalproject-hccp.onrender.com/api' // make sure to change this later
...
if (loginForm) {
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      // Save the JWT token in localStorage so we can attach it to every future price request
      // Without this token, the backend will reject requests with 401 Unauthorized
      localStorage.setItem('token', data.token)

      // Redirect to the dashboard where the user can manage their prices
